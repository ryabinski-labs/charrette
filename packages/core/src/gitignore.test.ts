...
function repo(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "harness-ignore-"));
...
describe("ensureIgnored", () => {
  ...
  it("writes nothing when a parent rule already ignores the path", () => {
    ...
    // harness must not add a redundant line to someone else's repo.
    writeFileSync(path.join(dir, ".gitignore"), ".harness*\n");
    expect(ensureIgnored(dir, ".harness/")).toBe(false);
    expect(read(dir)).toBe(".harness*\n");
  });
