...
describe("SHA-bound authorized release merge", () => {
  const original = () => ({ state: "open", draft: false, head: { ref: "harness/r/main", sha: "tested", repo: { full_name: "owner/repo" } }, base: { ref: "main" } });
  ...
  it("binds GitHub's compare-and-set merge to the exact validated head", async () => {
    const f = fixture();
    expect(await f.github.mergeApprovedPR(7, "harness/r/main", "main", "tested")).toBe(true);
  ...
  it.each(["head", "base", "sha", "fork", "draft", "closed"])("refuses an unexpected %s without attempting merge", async (change) => {
    ...
    expect(await f.github.mergeApprovedPR(7, "harness/r/main", "main", "tested")).toBe(false);
    expect(f.merge).not.toHaveBeenCalled();
  });
  it("does not bypass unavailable access or server-side protection", async () => {
    expect(await new GitHubAdapter(undefined, undefined).mergeApprovedPR(7, "harness/r/main", "main", "tested")).toBe(false);
    const f = fixture();
    expect(await f.github.mergeApprovedPR(7, "harness/r/main", "main", "")).toBe(false);
